...
There is no "light mode" on the homepage — the dark canvas is the product, and the only time the palette inverts is when a single story tile takes a yellow or yellow fill. The depth is almost entirely flat: **hairline 1px borders** (`#ffffff`, `#F3D23B`, or `#5200ff`) do the work that shadows would do on a Material-flavored site. Every container is either `#131313` with a 1px outline, a fully saturated accent block, or a slate-gray `#2d2d2d` secondary surface.

**Key Characteristics:**
- Near-black editorial canvas (`#131313`) as the default surface — no light mode on the homepage
- acid-yellow `#F3D23B` + ultraviolet `#5200ff` as hazard-tape accents, never quiet background wash
...
- Fully saturated color-block story tiles (yellow, purple, yellow, pink, orange, electric blue) on a dark page
- Timeline "StoryStream" feed with mono uppercase timestamps rather than a traditional magazine grid
- Flat depth — 1px borders in white, yellow, purple do the work that shadows would do elsewhere

## 2. Color Palette & Roles

### Primary (Brand Hazards)
- **Pokémon Yellow** (`#F3D23B`): The Verge's signature acid-yellow accent. Used as CTA button fill, link underlines, active tab borders, and high-attention story-tile backgrounds. Treat it as the visual equivalent of neon safety paint — applied sparingly to the most important element on screen.
...
### Secondary & Accent
- **Console Yellow Border** (`#CFA613`): A darker variant of the pokemon yellow used on card outlines and button borders where pure yellow would over-saturate.
- **Deep Link Blue** (`#3860be`): The link *hover* color — the one moment blue appears on the site. It replaces yellow/white/black on hover across every link style.
...
- **Absolute Black** (`#000000`): Reserved for text on the yellow/yellow/white tiles — the only place it appears.
...
- **Inverted Text** (`#131313`): Used only on accent tiles (yellow, yellow, white) to keep contrast legible.
...
### Gradient System
The Verge uses **zero decorative gradients**. The only gradient-like treatment is the transition from a saturated accent story tile (yellow/purple/yellow) back to the `#131313` canvas between rows. Color is applied in solid blocks, not as washes. This is a deliberate choice — the site's hazard-tape visual identity would dissolve if anything faded.
...
**Primary — Pokémon Yellow Pill**
- Background: `#F3D23B` (Pokémon Yellow)
...
**Tertiary — Outlined Yellow**
- Background: transparent
- Text: `#F3D23B`, PolySans Mono 12px / 600 UPPERCASE, 1.5px tracking
- Border: `1px solid #F3D23B`
- Border radius: `40px` — larger pill for secondary outline style
- Padding: ~`10px 20px`
- Hover: inverts to yellow fill, black text
...
- Background: saturated accent (`#F3D23B`, `#5200ff`, yellow, etc.)
...
**StoryStream Tile**
- Background: either `#131313` + 1px white border, OR a saturated accent fill (yellow, purple, yellow, pink, orange, white)
- Border radius: `20px` (standard) or `24px` (feature)
- Border: `1px solid #ffffff` (on dark) or `0px 0px 1px solid #F3D23B` (on yellow) or nothing (on saturated fill)
...
- **Focus**: border transitions to `#F3D23B` (pokemon yellow) with optional `1px solid #5200ff` inner ring on deep focus. No glow.
...
- **Top nav**: thin `#131313` bar with the Verge wordmark (Manuka) left-aligned, a search icon and a few UPPERCASE mono category links (12–14px, PolySans Mono, 1.5–1.8px tracking), and a single yellow-pill CTA (usually "Subscribe") pinned right.
...
- **Active section**: marked by a 1px yellow underline (inset box-shadow `0px -1px 0px 0px inset #F3D23B`)
...
| 3 | `1px solid #F3D23B` hairline | Active button outlines, focused story tiles |
...
| 6 | `0px -1px 0px 0px inset` (yellow/black/white) | Active tab underline — a signature Verge move |
| 7 | Saturated accent fill (`#F3D23B`, `#5200ff`, white, yellow, pink) | Story-tile elevation via color, not shadow |

The Verge's depth philosophy is **color-as-elevation**. When something needs to stand out, it doesn't get a shadow — it gets a yellow fill or a 1px hazard-color border. There are 14 shadow entries in the extracted tokens, but all of them are either inset underlines (0px -1px inset) or near-transparent 1px rings — none of them are traditional elevation shadows. The `#131313` canvas stays perfectly flat throughout, and hierarchy is carried by color saturation.

### Decorative Depth
- **1px inset underline** on active tabs/nav links (yellow, black, or white depending on context)
...
- **Do** use Pokémon Yellow (`#F3D23B`) and Verge Ultraviolet (`#5200ff`) as hazard accents — buttons, borders, active states, and saturated color-block tiles.
...
- **Do** use saturated color-block tiles (yellow, purple, yellow, pink, orange, white) to elevate a story — never a drop shadow.
...
- **Don't** let yellow and ultraviolet appear as background washes — they're hazard accents, not canvas tints.
- **Don't** use gradients anywhere. The system is solid color blocks only.
- **Don't** introduce new accent colors outside the declared yellow / purple / yellow / pink / orange tile palette.
...
- **Don't** use `#F3D23B` text on a `#131313` background at under 16px — the contrast vibrates at small sizes.
...
| Small Desktop | 1024–1179px | Full 3–4 column editorial grid, yellow pill CTA restored to nav |
...
- **Primary CTA**: "Pokémon Yellow (`#F3D23B`)"
...
- **Card Border**: "Hazard White (`#ffffff`)" hairline on dark, "Console Yellow Border (`#CFA613`)" on yellow variants
- **Link Hover**: "Deep Link Blue (`#3860be`)"

### Example Component Prompts
1. *"Create a StoryStream timeline item on a `#131313` canvas: a 20px-radius rectangle with a 1px solid `#ffffff` border, a PolySans Mono 11px / 600 / UPPERCASE / 1.1px tracking timestamp on the left rail, a 12px PolySans UPPERCASE kicker in yellow (`#F3D23B`), and a 24px / 700 PolySans headline in white below. No shadow, no lift — hover only shifts the headline color to `#3860be`."*
2. *"Design a primary subscribe button with a Pokémon Yellow (`#F3D23B`) fill, black text in PolySans Mono 12px / 600 / UPPERCASE / 1.5px tracking, 24px border radius, 10px × 24px padding. Hover state shifts to `rgba(255, 255, 255, 0.2)` background with a 1px `#c2c2c2` ring shadow, 180ms ease."*
...
5. **Audit accent usage.** Yellow and ultraviolet should appear as hazard accents — buttons, 1px borders, active underlines, saturated tile fills. If they're appearing as background washes or gradient fades, correct to solid blocks.
